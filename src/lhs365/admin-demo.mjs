...
export function adminDemo() {
  const people = Array.from({ length: 40 }, (_, i) => {const year=8+i%6;return { id: `demo-${i}`, name: `${i < 28 ? "Student" : "Staff"} ${String(i + 1).padStart(2, "0")}`, kind: i < 28 ? "student" : "staff", house: HOUSE_NAMES[i % 3], yearGroup: i < 28 ? `Year ${year}` : "Staff", formGroup:i<28?`${year}${i%2?"B":"A"}`:["8A","9B","10A","11B","12A","13B"][i%6], active: true };});
