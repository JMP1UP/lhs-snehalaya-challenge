...
{"name":"Full name","email":"school email","kind":"student or staff","house":"Beaumanor, Bradgate, Charnwood or None","yearGroup":"EYFS or Year 1–13","formGroup":"Tutor/form group","active":true,"openDay":{"role":"Tour Guide","time":"When to arrive","location":"Meeting point","lead":"Staff lead","instructions":"Short briefing"}}
...
function RosterImport({data,onSave,onVeracross,open=false}) {
  ...
  return <details className="admin-card roster-manager" id="roster-management" open={open || undefined}><summary>{data.people.length ? "Manage the school roster" : "Upload the school roster"}</summary>
    ...
    <label htmlFor="roster-paste">Or paste the roster</label><textarea id="roster-paste" value={raw} onChange={event=>setRaw(event.target.value)} placeholder={'Name,Email,Role,House,Year group,Form group,Open Day role,Open Day time,Open Day meeting point,Open Day staff lead,Open Day instructions\nAlex Example,alex.example@leicesterhigh.co.uk,student,Bradgate,Year 8,8A,Tour Guide,9:00 am,Main Hall,Ms Example,Welcome your family and collect a route.'}/>
    <button type="button" className="button secondary" disabled={busy} onClick={()=>inspect(raw)}>Check roster</button>
    {draft && <div className="roster-confirm"><p><strong>{draft.length} people ready to import</strong> · {draft.filter(p=>p.kind==="student").length} students · {draft.filter(p=>p.kind==="staff").length} staff · {draft.filter(p=>p.active).length} active · {draft.filter(p=>p.openDay?.role).length} Open Day missions</p>
