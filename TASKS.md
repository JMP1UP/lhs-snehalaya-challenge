# Current priorities — LHS Snehalaya Challenge

## Now
- [x] Establish 25Thirty Learning design standards in `docs/design.md`.
- [x] Document challenge entry and house leaderboard user flows in `docs/user-flows.md`.
- [ ] Audit React component styling (`src/index.css`) for 25Thirty Navy header and Inter typography.
- [ ] Verify accessibility of progress meters and keyboard focus.

## Next
- [ ] Refine house progress bar visual contrast.
- [ ] Ensure action buttons use direct verbs ("Log activity", "View progress").

## Later
- [ ] Add printable house milestone certificate generator.

## Blocked
- None.

## Recently completed
- Design documentation and user flow alignment.
