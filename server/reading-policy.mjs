...
export function authorisedIdentity(token, adminEmails) {
  if (!token?.uid || token.firebase?.sign_in_provider !== "microsoft.com" || typeof token.email !== "string" || !/^[a-z0-9._%+-]+@leicesterhigh\.co\.uk$/i.test(token.email)) {
    const error = new Error("Use a Leicester High Microsoft account."); error.status = 403; throw error;
  }
