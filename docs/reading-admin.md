...
Readers can remove an incorrectly logged book through a two-step inline confirmation. The server verifies personal ownership or membership of the linked family before deleting it, and the book's pages and completion are removed from aggregate totals.
...
- `VERACROSS_SCOPES`: `students:list staff_faculty:list`. Optional URL overrides are `VERACROSS_AUTH_URL` and `VERACROSS_API_BASE_URL`.
...
Configure the Microsoft provider to the school tenant and authorise the deployment hostname in Firebase Auth. The client tenant parameter is not the only access-control boundary: the API verifies a Firebase ID token with revocation checks, a Microsoft sign-in provider and a Leicester High email supplied by that provider. Microsoft/Firebase does not consistently issue an `email_verified` claim for this provider, so that optional claim is not required. Any authenticated school account may use the reading challenge; the roster supplies school grouping and participation denominators, not basic access. Admin access still requires the server allowlist, and staff aggregate access requires an active staff roster record. Verify actual tenant isolation in the pilot. In live mode, missing configuration fails closed; it never falls back to local preview storage. Signing out clears the displayed report/bookshelf. Admin names and roster records are not public endpoints. The unauthenticated `summary` resource exposes only whole-school page, height, reader and finished-book totals; it is edge-cached briefly and never reads or returns the roster.
...
`settings/roster` holds one versioned document: people (up to 1,000), complete flag, version, updatedAt. Each person has email, name, kind (student/staff), house, yearGroup, formGroup and active. IDs are hashes of normalised emails. Students require EYFS or Year 1–13 and retain the complete Veracross tutor-group label; a genuinely blank tutor field is shown as Unassigned instead of dropping the pupil. Staff can use house None and may have a form group. Houses: Beaumanor, Bradgate, Charnwood. Books are linked to these server-generated IDs. `members/{id}` tracks the 200-book pilot limit. `books/{id}` contains validated current/start/total pages and server-dated positive deltas. The report supports up to 10,000 books and fails visibly beyond that limit.
